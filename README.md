...
## Events
* [Participant Joins the Hub](#participant-joins-the-scheme)
...
A participant decides to deposit 100 units collateral into the Hub hub.

### Entities
* Bank (External)

### Accounts
* Hub Credit Line - Hub operator liquidity extension to a Participant
...
* Participant Collateral A - Deposit collateral into the Hub
* Participant A Fees - The account tracking the transfer fees charged by the scheme against A

### Activity
A cash deposit is made at a bank that supports loading of assets into the Hub hub.
...
#### Hub decides to charge Fees on Deposit of Collateral:
...
#### Hub extends Credit to A as a function of A's collateral deposit event amount:
> (A has a net debit cap of 150 according to rules of scheme)

```
DR Hub Credit Line                              50
...
* A's Liquidity has a CR balance of; `100 + 10 + 50 = 160`
    ...
    * `50` of the `160` units were made available by the Hub's credit line
...
* A's Fees has a DR balance of; `0 + 10 - 10 = 0`
  * `10` units fee charge for deposit, followed by `10` fee units credited to Hub Fees 
* Hub's Liquidity has a DR balance of; `0 + 50 = 50`
    * Hub extends `50` units of liquidity to A
    * Note that the scheme limit is set at 150 units liquidity cap, current usage is at 33%
* Hub's Collateral has a CR balance of; `0 + 120 - 120 = 0`
...
* A's Fees has a DR balance of; `0 + 10 - 10 = 0`
  * `10` units fee charge for transfer, followed by `10` fee units credited to Hub Fees
* Participant A Payable to Hub has a CR balance of; `10 + 10 + 100 - 100 = 20`
